...
import { AdminBasicService } from '../../../../../services/AdminBasic.service';
...
@Component({
  ...
  templateUrl: './Exercise-admin-Basic.component.html',
})
export class ExerciseAdminBasicComponent implements OnInit {
  exercises: any[] = [];
  ...
  constructor(private adminBasicService: AdminBasicService) {}
  ...
  loadExercises(): void {
    this.adminBasicService.getAllExercises().subscribe({
      next: (data: any[]) => {
        this.exercises = data;
      },
      error: (err: any) => {
  ...
  // Guardar ejercicio
  saveExercise(): void {
    const formData = this.adminBasicService.createExerciseFormData(
      {
    ...
    if (this.isEditing) {
      ...
      this.adminBasicService.updateExercise(this.currentExercise.id, formData).subscribe({
      ...
      this.adminBasicService.createExercise(formData).subscribe({
        next: (newExercise) => {
...
deleteExercise(exerciseId: number): void {
    if (confirm('¿Estás seguro de que deseas eliminar este ejercicio?')) {
      this.adminBasicService.deleteExercise(exerciseId).subscribe({
        next: () => {
